...
class ModelSkeleton:
  ...
  def filter_prediction(self, boxes, probs, cls_idx):
    ...
    if mc.TOP_N_DETECTION < len(probs) and mc.TOP_N_DETECTION > 0:
      ...
      print(" IF mc.PROB_THRESH = ", mc.PROB_THRESH)
      print("mc.TOP_N_DETECTION = ", mc.TOP_N_DETECTION)
      ...
      print(" ELSE mc.PROB_THRESH", mc.PROB_THRESH)
    ...
    for c in range(mc.CLASSES):
      ...
      for i in range(len(keep)):
        if keep[i]:
          ...
          final_order.append(order[idx_per_class[i]])
    return final_boxes, final_probs, final_cls_idx, final_order
